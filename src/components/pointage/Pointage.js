...
import {
  // detectFaceAndComputeEmbedding,getLoadedModels,
  loadModels,
...
const Pointage = ({ user }) => {
  ...
  // 🔮 NOUVEAUX ÉTATS POUR LE POINTAGE
  const [pointageType, setPointageType] = useState("");
  const [pointageTime, setPointageTime] = useState("");
  ...
  const checkFaceQuality = useCallback(async () => {
    ...
    try {
      ...
      const x = Math.max(
        0,
        Math.min(100, 50 - ((box.x + box.width / 2) / 640) * 100)
      );
      const y = Math.max(
        0,
        Math.min(100, 50 - ((box.y + box.height / 2) / 480) * 100)
      );
    ...
    try {
      ...
      if (employesAvecEmbedding.length === 0) {
        setLastResult({
          type: "warning",
          message:
            "Aucune signature neurale enregistrée. Procédez à l'enrôlement.",
        });
      } else {
  ...
  const determinerTypePointage = async (employeId) => {
    try {
      const q = query(
      ...
      if (dernierPointage) {
        ...
        // Si le dernier pointage était une entrée et qu'il y a moins de 4 heures
        if (dernierPointage.type === "entrée" && diffHeures < 4) {
          type = "sortie";
        }
        // Si le dernier pointage était une sortie, c'est une nouvelle entrée
        else if (dernierPointage.type === "sortie") {
          type = "entrée";
        }
      }
      ...
      console.error("❌ Erreur détermination type pointage:", error);
      return "entrée"; // Par défaut
    }
  };
  ...
  const captureAndRecognize = async () => {
    ...
    try {
      ...
      // 🔮 DÉTERMINER LE TYPE DE POINTAGE ET L'HEURE
      const typePointage = await determinerTypePointage(result.bestMatch.id);
      const heurePointage = new Date().toLocaleString("fr-FR", {
        weekday: "long",
        year: "numeric",
        month: "long",
        day: "numeric",
      ...
      setPointageType(typePointage);
      setPointageTime(heurePointage);
  ...
  const enregistrerPointage = async (
    employe,
    confidence,
    photoCapture,
    type
  ...
  // 🔮 COULEUR POUR LE TYPE DE POINTAGE
  const getPointageColor = (type) => {
    return type === "entrée" ? "#10b981" : "#ef4444";
  };

  // 🔮 ICONE POUR LE TYPE DE POINTAGE
  const getPointageIcon = (type) => {
    return type === "entrée" ? "🟢" : "🔴";
  };
              ...
              <div className="modal-content">
                <div className="employee-info">
                  ...
                  <div className="employee-details">
                    ...
                    {/* 🔮 AFFICHAGE DU TYPE DE POINTAGE ET HEURE */}
                    <div className="pointage-info">
                      <div
                        className="pointage-type-badge"
                        style={{
                          backgroundColor: getPointageColor(pointageType),
                          color: "white",
                        }}
                      >
                        {getPointageIcon(pointageType)} Pointage{" "}
                        {pointageType.toUpperCase()}
                      </div>
                      <div className="pointage-time">{pointageTime}</div>
                    </div>
                  </div>
                </div>

                <div className="success-message">
                  <p>Employé pointé avec succès.</p>
              ...
              <div className="modal-content">
                <div className="unrecognized-content">
                  ...
                  <div className="unrecognized-text">
                    ...
                    <p className="unrecognized-note">
                      <strong>Pour améliorer la reconnaissance :</strong>
